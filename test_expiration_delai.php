...
echo "=== TEST EXPIRATION DÉLAI ET PASSAGE AU SUIVANT ===\n\n";

// 1. ÉTAT ACTUEL
echo "1. ÉTAT ACTUEL AVANT EXPIRATION\n";
echo "=================================\n";

$lot = $lotRepository->find(5);
$user3 = $userRepository->find(3); // Premier en file d'attente
$user4 = $userRepository->find(4); // Deuxième en file d'attente
...
// 2. SIMULATION EXPIRATION DÉLAI
echo "2. SIMULATION EXPIRATION DÉLAI\n";
echo "===============================\n";

// Trouver l'utilisateur en attente de validation (user ID 3)
...
if ($premierEnAttente) {
    testResult(
        ...
        true,
        "Statut: {$premierEnAttente->getStatut()}, Expire: " . ($premierEnAttente->getExpiresAt() ? $premierEnAttente->getExpiresAt()->format('H:i:s') : 'Non défini')
    );
    ...
    testResult(
        ...
        true,
        "Statut: {$premierEnAttente->getStatut()}, Expiré à: {$premierEnAttente->getExpiredAt()->format('H:i:s')}"
    );
