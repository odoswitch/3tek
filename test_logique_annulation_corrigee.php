...
// 6. VÉRIFICATION DE LA FILE D'ATTENTE
echo "6. VÉRIFICATION DE LA FILE D'ATTENTE\n";
echo "======================================\n";

$toutesFilesAttente = $fileAttenteRepository->createQueryBuilder('f')
    ->where('f.lot = :lot')
    ->setParameter('lot', $lot)
    ->orderBy('f.position', 'ASC')
    ->getQuery()
...
echo "📋 État de la file d'attente après annulation:\n";
foreach ($toutesFilesAttente as $file) {
    echo "   - Position {$file->getPosition()}: {$file->getUser()->getEmail()} (Statut: {$file->getStatut()})\n";
}

testResult(
    "File d'attente maintenue",
    count($toutesFilesAttente) >= 2,
    count($toutesFilesAttente) . " utilisateur(s) en file d'attente"
);
