...
echo "\n🔧 ÉTAPE 2: Création d'utilisateurs pour le test utilisateur...\n";

// Créer des utilisateurs pour le test
$createUser1Command = "php bin/console doctrine:query:sql \"INSERT INTO user (email, name, lastname, password, roles, office, phone, address, code, ville, pays, is_verified) VALUES ('testuser1@example.com', 'TestUser1', 'User', '\$2y\$13\$test', '[]', 'Test Office', '0123456789', 'Test Address', '12345', 'Test City', 'Test Country', 1)\"";
exec($createUser1Command, $createUser1Output, $createUser1ReturnCode);

$createUser2Command = "php bin/console doctrine:query:sql \"INSERT INTO user (email, name, lastname, password, roles, office, phone, address, code, ville, pays, is_verified) VALUES ('testuser2@example.com', 'TestUser2', 'User', '\$2y\$13\$test', '[]', 'Test Office', '0123456789', 'Test Address', '12345', 'Test City', 'Test Country', 1)\"";
exec($createUser2Command, $createUser2Output, $createUser2ReturnCode);

$createUser3Command = "php bin/console doctrine:query:sql \"INSERT INTO user (email, name, lastname, password, roles, office, phone, address, code, ville, pays, is_verified) VALUES ('testuser3@example.com', 'TestUser3', 'User', '\$2y\$13\$test', '[]', 'Test Office', '0123456789', 'Test Address', '12345', 'Test City', 'Test Country', 1)\"";
...
$getUser1IdCommand = "php bin/console doctrine:query:sql \"SELECT id FROM user WHERE email = 'testuser1@example.com'\"";
exec($getUser1IdCommand, $getUser1IdOutput, $getUser1IdReturnCode);

$getUser2IdCommand = "php bin/console doctrine:query:sql \"SELECT id FROM user WHERE email = 'testuser2@example.com'\"";
exec($getUser2IdCommand, $getUser2IdOutput, $getUser2IdReturnCode);

$getUser3IdCommand = "php bin/console doctrine:query:sql \"SELECT id FROM user WHERE email = 'testuser3@example.com'\"";
...
echo "\n🎯 SCÉNARIO UTILISATEUR CRÉÉ !\n";
echo "Maintenant vous pouvez :\n";
echo "1. Vérifier que le lot 'Serveurs' est en statut 'réservé' sur l'interface utilisateur\n";
echo "2. Vérifier que les utilisateurs User2 et User3 voient le bouton 'Rejoindre la file d'attente'\n";
echo "3. Aller sur http://localhost:8080/admin\n";
echo "4. Supprimer la commande créée (ID du lot: 14)\n";
echo "5. Vérifier que le lot passe à 'disponible'\n";
echo "6. Vérifier que User2 (premier en file) est notifié\n";
echo "7. Vérifier les logs avec: docker compose exec php php check_debug_logs.php\n\n";

echo "📊 DONNÉES CRÉÉES:\n";
echo "- Lot 'Serveurs' (ID: 14) en statut 'réservé' par User1 (ID: $user1Id)\n";
echo "- User2 ID: $user2Id (premier en file d'attente)\n";
echo "- User3 ID: $user3Id (deuxième en file d'attente)\n";
echo "- Commande créée depuis l'interface utilisateur\n";
echo "- Files d'attente créées\n\n";
...
echo "2. Vérifiez que les autres utilisateurs voient le bouton 'Rejoindre la file d'attente'\n";
echo "3. Puis supprimez la commande via l'admin et vérifiez qu'il passe à 'disponible'\n";
