...
foreach ($testUsers as $index => $user) {
    ...
    // Vérifier le statut dans la file d'attente
    $filesUser = $fileAttenteRepository->createQueryBuilder('f')
        ->where('f.lot = :lot')
    ...
    if (!empty($filesUser)) {
        foreach ($filesUser as $file) {
            echo "   - Position: {$file->getPosition()}\n";
            echo "   - Statut: {$file->getStatut()}\n";
            if ($file->getExpiresAt()) {
                echo "   - Délai d'expiration: {$file->getExpiresAt()->format('d/m/Y H:i')}\n";
            }
        }
    } else {
        echo "   - Pas en file d'attente\n";
    }
    echo "\n";
}
...
echo "   - Entité FileAttente étendue avec délais\n";
echo "   - Processus équitable et efficace\n";
echo "   - Système de notification automatique\n";
