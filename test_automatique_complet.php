...
$createLotCommand = "php bin/console doctrine:query:sql \"INSERT INTO lot (name, description, prix, quantite, statut) VALUES ('Lot Test Final', 'Description du lot test final', 200.00, 1, 'disponible')\"";
...
$getLotIdCommand = "php bin/console doctrine:query:sql \"SELECT id FROM lot WHERE name = 'Lot Test Final' ORDER BY id DESC LIMIT 1\"";
...
$createUser1Command = "php bin/console doctrine:query:sql \"INSERT INTO user (email, name, lastname, password, roles, office, phone, address, code, ville, pays, is_verified) VALUES ('testfinal1@example.com', 'TestFinal', 'User1', '\$2y\$13\$test', '[]', 'Test Office', '0123456789', 'Test Address', '12345', 'Test City', 'Test Country', 1)\"";
exec($createUser1Command, $createUser1Output, $createUser1ReturnCode);

$createUser2Command = "php bin/console doctrine:query:sql \"INSERT INTO user (email, name, lastname, password, roles, office, phone, address, code, ville, pays, is_verified) VALUES ('testfinal2@example.com', 'TestFinal', 'User2', '\$2y\$13\$test', '[]', 'Test Office', '0123456789', 'Test Address', '12345', 'Test City', 'Test Country', 1)\"";
...
$getUser1IdCommand = "php bin/console doctrine:query:sql \"SELECT id FROM user WHERE email = 'testfinal1@example.com'\"";
exec($getUser1IdCommand, $getUser1IdOutput, $getUser1IdReturnCode);

$getUser2IdCommand = "php bin/console doctrine:query:sql \"SELECT id FROM user WHERE email = 'testfinal2@example.com'\"";
...
$createCommandeCommand = "php bin/console doctrine:query:sql \"INSERT INTO commande (user_id, lot_id, quantite, prix_unitaire, prix_total, statut) VALUES ($user1Id, $lotId, 1, 200.00, 200.00, 'reserve')\"";
...
$updateLotCommand = "php bin/console doctrine:query:sql \"UPDATE lot SET statut = 'reserve', reserve_par_id = $user1Id WHERE id = $lotId\"";
...
echo "\n🔧 ÉTAPE 8: Vérification de l'état avant suppression...\n";

// Vérifier l'état avant suppression
$checkBeforeCommand = "php bin/console doctrine:query:sql \"SELECT id, name, statut, quantite, reserve_par_id FROM lot WHERE id = $lotId\"";
exec($checkBeforeCommand, $checkBeforeOutput, $checkBeforeReturnCode);

if ($checkBeforeReturnCode === 0 && !empty($checkBeforeOutput)) {
    echo "État du lot avant suppression :\n";
    foreach ($checkBeforeOutput as $line) {
...
echo "1. Vérifier que le lot est en statut 'réservé' sur l'interface utilisateur\n";
...
echo "📊 DONNÉES CRÉÉES:\n";
echo "- Lot ID: $lotId (en statut 'réservé')\n";
...
echo "Vérifiez d'abord que le lot est bien en statut 'réservé' sur l'interface utilisateur\n";
