<?php
echo "=== TEST LOGIQUE COMPLÈTE ===\n\n";

echo "🔧 ÉTAPE 1: Création d'un lot de test...\n";

// Créer un lot de test
$createLotCommand = "php bin/console doctrine:query:sql \"INSERT INTO lot (name, description, prix, quantite, statut) VALUES ('Lot Test Automatique', 'Description du lot test automatique', 150.00, 1, 'disponible')\"";
...
$getLotIdCommand = "php bin/console doctrine:query:sql \"SELECT id FROM lot WHERE name = 'Lot Test Automatique' ORDER BY id DESC LIMIT 1\"";
...
$createUser1Command = "php bin/console doctrine:query:sql \"INSERT INTO user (email, name, lastname, password, roles, office, phone, address, code, ville, pays, is_verified) VALUES ('testauto1@example.com', 'TestAuto', 'User1', '\$2y\$13\$test', '[]', 'Test Office', '0123456789', 'Test Address', '12345', 'Test City', 'Test Country', 1)\"";
exec($createUser1Command, $createUser1Output, $createUser1ReturnCode);

$createUser2Command = "php bin/console doctrine:query:sql \"INSERT INTO user (email, name, lastname, password, roles, office, phone, address, code, ville, pays, is_verified) VALUES ('testauto2@example.com', 'TestAuto', 'User2', '\$2y\$13\$test', '[]', 'Test Office', '0123456789', 'Test Address', '12345', 'Test City', 'Test Country', 1)\"";
...
$getUser1IdCommand = "php bin/console doctrine:query:sql \"SELECT id FROM user WHERE email = 'testauto1@example.com'\"";
exec($getUser1IdCommand, $getUser1IdOutput, $getUser1IdReturnCode);

$getUser2IdCommand = "php bin/console doctrine:query:sql \"SELECT id FROM user WHERE email = 'testauto2@example.com'\"";
...
$createCommandeCommand = "php bin/console doctrine:query:sql \"INSERT INTO commande (user_id, lot_id, quantite, prix_unitaire, prix_total, statut) VALUES ($user1Id, $lotId, 1, 150.00, 150.00, 'reserve')\"";
...
echo "2. Supprimer la commande créée (ID du lot: $lotId)\n";
